...
import { constitutionActivities } from '@/data/constitutionActivities';
import { Switch } from '@/components/ui/switch';
import { Label } from '@/components/ui/label';
import { cn } from '@/lib/utils';
import { PlusCircle, MinusCircle, Edit, Folder } from 'lucide-react';
import {
  Accordion,
  AccordionContent,
  AccordionItem,
...
export const ConstitutionActivityDashboard: React.FC = () => {
  const [showCompleted, setShowCompleted] = useState(false);
  const [isOpen, setIsOpen] = useState(true);

  // Calculate total stats
  const totalStats = constitutionActivities.reduce(
    (acc, group) => ({
        ...
        <AccordionItem value="todos" className="border rounded-lg bg-white shadow-sm">
          ...
          <AccordionContent className="px-6 pt-4 pb-6">
            <div className="space-y-4">
              {constitutionActivities.map((group) => (
                <ActivityGroup
                  key={group.id}
                  group={group}
