...
import { financeActivities } from '@/data/financeActivities';
import { Switch } from '@/components/ui/switch';
import { Label } from '@/components/ui/label';
import { cn } from '@/lib/utils';
import { PlusCircle, MinusCircle, Calculator, CheckCircle, Upload } from 'lucide-react';
import {
  Accordion,
  AccordionContent,
  AccordionItem,
...
const StatBox: React.FC<StatBoxProps> = ({ value, label, className }) => (
  <div className={cn("px-3 py-1.5 rounded-lg text-center", className)}>
...
const ActivityItem: React.FC<ActivityItemProps> = ({ activity, isCompleted = false }) => {
  const actionStyles = {
    ...
    'audit': 'bg-blue-500 text-white hover:bg-blue-600',
    'authorise': 'bg-blue-500 text-white hover:bg-blue-600',
...
export const FinanceActivityDashboard: React.FC = () => {
  const [showCompleted, setShowCompleted] = useState(false);
  const [isOpen, setIsOpen] = useState(true);

  // Calculate total stats
  const totalStats = financeActivities.reduce(
    (acc, group) => ({
