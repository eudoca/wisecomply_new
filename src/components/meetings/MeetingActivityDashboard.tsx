...
import { meetingActivities } from '@/data/meetingActivities';
import { Switch } from '@/components/ui/switch';
import { Label } from '@/components/ui/label';
import { cn } from '@/lib/utils';
import { PlusCircle, MinusCircle, Calendar, Users, FileText, Folder } from 'lucide-react';
import {
  Accordion,
  AccordionContent,
  AccordionItem,
...
const ActivityItem: React.FC<ActivityItemProps> = ({ activity, isCompleted = false }) => {
  const actionStyles = {
    ...
    'schedule': 'bg-blue-500 text-white hover:bg-blue-600',
    'notify': 'bg-blue-500 text-white hover:bg-blue-600',
    'prepare': 'bg-blue-500 text-white hover:bg-blue-600',
    'verify': 'bg-blue-500 text-white hover:bg-blue-600',
    'record': 'bg-blue-500 text-white hover:bg-blue-600',
    'escalate': 'bg-blue-500 text-white hover:bg-blue-600',
...
export const MeetingActivityDashboard: React.FC = () => {
  const [showCompleted, setShowCompleted] = useState(false);
  const [isOpen, setIsOpen] = useState(true);

  // Calculate total stats
  const totalStats = meetingActivities.reduce(
    (acc, group) => ({
