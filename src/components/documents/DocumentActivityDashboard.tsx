...
import { documentActivities } from '@/data/documentActivities';
import { Switch } from '@/components/ui/switch';
import { Label } from '@/components/ui/label';
import { cn } from '@/lib/utils';
import { PlusCircle, MinusCircle, FileCheck, Archive, Eye } from 'lucide-react';
import {
  Accordion,
  AccordionContent,
  AccordionItem,
...
const ActivityItem: React.FC<ActivityItemProps> = ({ activity, isCompleted = false }) => {
  const actionStyles = {
    ...
    'document': 'bg-blue-500 text-white hover:bg-blue-600',
    'respond': 'bg-blue-500 text-white hover:bg-blue-600',
    'provide': 'bg-blue-500 text-white hover:bg-blue-600'
  };
...
export const DocumentActivityDashboard: React.FC = () => {
  const [showCompleted, setShowCompleted] = useState(false);
  const [isOpen, setIsOpen] = useState(true);

  // Calculate total stats
  const totalStats = documentActivities.reduce(
    (acc, group) => ({
